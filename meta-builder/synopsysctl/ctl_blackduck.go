...
type BlackduckCRSpecBuilderFromCobraFlags struct {
	blackDuckSpec                 *synopsysv1.BlackduckSpec
	Size                          string
	Version                       string
...
func NewBlackduckCRSpecBuilderFromCobraFlags() *BlackduckCRSpecBuilderFromCobraFlags {
	return &BlackduckCRSpecBuilderFromCobraFlags{
		blackDuckSpec: &synopsysv1.BlackduckSpec{},
	}
}

// GetCRSpec returns a pointer to the BlackDuckSpec as an interface{}
func (ctl *BlackduckCRSpecBuilderFromCobraFlags) GetCRSpec() interface{} {
	return *ctl.blackDuckSpec
}

// SetCRSpec sets the blackDuckSpec in the struct
func (ctl *BlackduckCRSpecBuilderFromCobraFlags) SetCRSpec(spec interface{}) error {
	convertedSpec, ok := spec.(synopsysv1.BlackduckSpec)
	if !ok {
...
func (ctl *BlackduckCRSpecBuilderFromCobraFlags) SetPredefinedCRSpec(specType string) error {
	switch specType {
	case EmptySpec:
		ctl.blackDuckSpec = &synopsysv1.BlackduckSpec{}
	case PersistentStorageLatestSpec:
		ctl.blackDuckSpec = utils.GetBlackDuckDefaultPersistentStorageLatest()
	case PersistentStorageV1Spec:
		ctl.blackDuckSpec = utils.GetBlackDuckDefaultPersistentStorageV1()
	case ExternalPersistentStorageLatestSpec:
		ctl.blackDuckSpec = utils.GetBlackDuckDefaultExternalPersistentStorageLatest()
	case ExternalPersistentStorageV1Spec:
		ctl.blackDuckSpec = utils.GetBlackDuckDefaultExternalPersistentStorageV1()
	case BDBASpec:
		ctl.blackDuckSpec = utils.GetBlackDuckDefaultBDBA()
	case EphemeralSpec:
		ctl.blackDuckSpec = utils.GetBlackDuckDefaultEphemeral()
	case EphemeralCustomAuthCASpec:
		ctl.blackDuckSpec = utils.GetBlackDuckDefaultEphemeralCustomAuthCA()
	case ExternalDBSpec:
		ctl.blackDuckSpec = utils.GetBlackDuckDefaultExternalDB()
	case IPV6DisabledSpec:
		ctl.blackDuckSpec = utils.GetBlackDuckDefaultIPV6Disabled()
	default:
...
func (ctl *BlackduckCRSpecBuilderFromCobraFlags) AddCRSpecFlagsToCommand(cmd *cobra.Command, master bool) {
	if master {
	...
	if master {
		cmd.Flags().StringVar(&ctl.ExposeService, "expose-ui", utils.NONE, "Service type of Black Duck webserver's user interface (NODEPORT|LOADBALANCER|OPENSHIFT|NONE)")
	} else {
...
func (ctl *BlackduckCRSpecBuilderFromCobraFlags) CheckValuesFromFlags(flagset *pflag.FlagSet) error {
	...
	if FlagWasSet(flagset, "expose-ui") {
		isValid := utils.IsExposeServiceValid(ctl.ExposeService)
		if !isValid {
			return fmt.Errorf("expose ui must be '%s', '%s', '%s' or '%s'", utils.NODEPORT, utils.LOADBALANCER, utils.OPENSHIFT, utils.NONE)
		}
	}
...
func (ctl *BlackduckCRSpecBuilderFromCobraFlags) SetCRSpecFieldByFlag(f *pflag.Flag) {
	if f.Changed {
		...
		case "external-postgres-host":
			if ctl.blackDuckSpec.ExternalPostgres == nil {
				ctl.blackDuckSpec.ExternalPostgres = &synopsysv1.PostgresExternalDBConfig{}
			}
		...
		case "external-postgres-admin-password":
			...
			ctl.blackDuckSpec.ExternalPostgres.PostgresAdminPassword = utils.Base64Encode([]byte(ctl.ExternalPostgresAdminPassword))
		case "external-postgres-user-password":
			if ctl.blackDuckSpec.ExternalPostgres == nil {
				ctl.blackDuckSpec.ExternalPostgres = &synopsysv1.PostgresExternalDBConfig{}
			}
			ctl.blackDuckSpec.ExternalPostgres.PostgresUserPassword = utils.Base64Encode([]byte(ctl.ExternalPostgresUserPassword))
		...
		case "pvc-file-path":
			data, err := utils.ReadFileData(ctl.PVCFilePath)
			if err != nil {
		...
		case "node-affinity-file-path":
			data, err := utils.ReadFileData(ctl.NodeAffinityFilePath)
			if err != nil {
				log.Errorf("failed to read node affinity file: %+v", err)
				return
			}
			nodeAffinities := map[string][]synopsysv1.NodeAffinity{}
		...
		case "postgres-claim-size":
			...
			ctl.blackDuckSpec.PVC = append(ctl.blackDuckSpec.PVC, synopsysv1.PVC{Name: "blackduck-postgres", Size: ctl.PostgresClaimSize}) // add postgres PVC if doesn't exist
		case "certificate-name":
			ctl.blackDuckSpec.CertificateName = ctl.CertificateName
		case "certificate-file-path":
			data, err := utils.ReadFileData(ctl.CertificateFilePath)
			if err != nil {
		...
		case "certificate-key-file-path":
			data, err := utils.ReadFileData(ctl.CertificateKeyFilePath)
			if err != nil {
				log.Errorf("failed to read certificate file: %+v", err)
				return
			}
		...
		case "proxy-certificate-file-path":
			data, err := utils.ReadFileData(ctl.ProxyCertificateFilePath)
			if err != nil {
				log.Errorf("failed to read certificate file: %+v", err)
				return
			}
		...
		case "auth-custom-ca-file-path":
			data, err := utils.ReadFileData(ctl.AuthCustomCAFilePath)
			if err != nil {
		...
		case "admin-password":
			ctl.blackDuckSpec.AdminPassword = utils.Base64Encode([]byte(ctl.AdminPassword))
		case "postgres-password":
			ctl.blackDuckSpec.PostgresPassword = utils.Base64Encode([]byte(ctl.PostgresPassword))
		case "user-password":
			ctl.blackDuckSpec.UserPassword = utils.Base64Encode([]byte(ctl.UserPassword))
		case "enable-binary-analysis":
			if ctl.EnableBinaryAnalysis {
				ctl.blackDuckSpec.Environs = utils.MergeEnvSlices([]string{"USE_BINARY_UPLOADS:1"}, ctl.blackDuckSpec.Environs)
			} else {
				ctl.blackDuckSpec.Environs = utils.MergeEnvSlices([]string{"USE_BINARY_UPLOADS:0"}, ctl.blackDuckSpec.Environs)
			}
		case "enable-source-code-upload":
			if ctl.EnableSourceCodeUpload {
				ctl.blackDuckSpec.Environs = utils.MergeEnvSlices([]string{"ENABLE_SOURCE_UPLOADS:true"}, ctl.blackDuckSpec.Environs)
			} else {
				ctl.blackDuckSpec.Environs = utils.MergeEnvSlices([]string{"ENABLE_SOURCE_UPLOADS:false"}, ctl.blackDuckSpec.Environs)
			}
		case "registry":

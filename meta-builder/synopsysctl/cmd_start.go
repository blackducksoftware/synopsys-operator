...
var startAlertCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		for _, altArg := range args {
			alertName, alertNamespace, _, err := getInstanceInfo(false, utils.AlertCRDName, utils.AlertName, namespace, altArg)
			if err != nil {
			...
			// Get the Alert
			currAlert, err := utils.GetAlert(restClient, alertNamespace, alertName, v1.GetOptions{})
			if err != nil {
			...
			// Update Alert
			_, err = utils.UpdateAlert(restClient, currAlert)
			if err != nil {
...
var startBlackDuckCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		for _, bdArg := range args {
			blackDuckName, blackDuckNamespace, _, err := getInstanceInfo(false, utils.BlackDuckCRDName, utils.BlackDuckName, namespace, bdArg)
			if err != nil {
				errors = append(errors, err)
				continue
			}
			...
			// Get the Black Duck
			currBlackDuck, err := utils.GetBlackduck(restClient, blackDuckNamespace, blackDuckName, v1.GetOptions{})
			if err != nil {
...
var startOpsSightCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		for _, opsArg := range args {
			opsSightName, opsSightNamespace, _, err := getInstanceInfo(false, utils.OpsSightCRDName, utils.OpsSightName, namespace, opsArg)
			if err != nil {
				errors = append(errors, err)
				continue
			}
			...
			// Get the OpsSight
			currOpsSight, err := utils.GetOpsSight(restClient, opsSightNamespace, opsSightName, v1.GetOptions{})
			if err != nil {
			...
			// Update OpsSight
			_, err = utils.UpdateOpsSight(restClient, currOpsSight)
			if err != nil {

...
func destroy(namespace string) error {
	// delete namespace
	isNamespaceExist, err := utils.CheckResourceNamespace(kubeClient, namespace, "", true)
	if isNamespaceExist {
		// get the synopsys operator configmap to get the crd names and cluster scope
		crds := make([]string, 0)
		var isClusterScoped bool
		cm, cmErr := utils.GetConfigMap(kubeClient, namespace, "synopsys-operator")
		if cmErr != nil {
			...
			if crdNames, ok := cmData["CrdNames"]; ok {
				crds = utils.StringToStringSlice(crdNames.(string), ",")
			}
		...
		if isClusterScoped {
			if err != nil {
				...
				isOwnerLabelExist := utils.IsOwnerLabelExistInNamespace(kubeClient, namespace)
				if isOwnerLabelExist {
					err = utils.DeleteNamespace(kubeClient, namespace)
					if err != nil {
	...
	clusterRoleBindings, _, err := utils.GetOperatorRoleBindings(kubeClient, namespace)
	if err != nil {
		log.Errorf("error getting role binding or cluster role binding due to %+v", err)
	}

	for _, clusterRoleBinding := range clusterRoleBindings {
		crb, err := utils.GetClusterRoleBinding(kubeClient, clusterRoleBinding)
		if err != nil {
		...
		for _, subject := range crb.Subjects {
			isExist := utils.IsSubjectExistForOtherNamespace(subject, namespace)
			if isExist {
		...
		if len(newSubjects) > 0 {
			...
			err := utils.DeleteClusterRoleBinding(kubeClient, clusterRoleBinding)
			if err != nil {
	...
	clusterRoles, _, err := utils.GetOperatorRoles(kubeClient, namespace)
	if err != nil {
		log.Errorf("error getting role or cluster role due to %+v", err)
	}

	crbs, err := utils.ListClusterRoleBindings(kubeClient, "app in (synopsys-operator,opssight)")

	for _, clusterRole := range clusterRoles {
		isExist := false
		// check whether the cluster role is referenced in any cluster role binding
		for _, crb := range crbs.Items {
			if utils.IsClusterRoleRefExistForOtherNamespace(crb.RoleRef, clusterRole, namespace, crb.Subjects) {
				isExist = true
			}
		}
		if !isExist {
			log.Infof("deleting cluster role '%s'", clusterRole)
			err := utils.DeleteClusterRole(kubeClient, clusterRole)
			if err != nil {
...
func isDeleteCrd(crd string, namespace string) error {
	...
	case utils.AlertCRDName:
		...
		crd, err := utils.GetCustomResourceDefinition(apiExtensionClient, utils.AlertCRDName)
		if err != nil {
			return fmt.Errorf("unable to get %s custom resource definition due to %+v", utils.AlertCRDName, err)
		}
	...
	case utils.BlackDuckCRDName:
		// check custom resource definition exist
		crd, err := utils.GetCustomResourceDefinition(apiExtensionClient, utils.BlackDuckCRDName)
		if err != nil {
			return fmt.Errorf("unable to get %s custom resource definition due to %+v", utils.BlackDuckCRDName, err)
		}
	...
	case utils.OpsSightCRDName:
		// check custom resource definition exist
		crd, err := utils.GetCustomResourceDefinition(apiExtensionClient, utils.OpsSightCRDName)
		if err != nil {
			return fmt.Errorf("unable to get %s custom resource definition due to %+v", utils.OpsSightCRDName, err)
		}
...
func deleteCrd(crd string, namespace string) {
	...
	if err != nil {
		...
		err := utils.DeleteCustomResourceDefinition(apiExtensionClient, crd)
		if err != nil {

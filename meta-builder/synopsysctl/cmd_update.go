...
//		if !isValid {
//			cmd.Help()
//			return nil, nil, fmt.Errorf("expose ui must be '%s', '%s', '%s' or '%s'", utils.NODEPORT, utils.LOADBALANCER, utils.OPENSHIFT, utils.NONE)
//		}
...
//		_, ok := crdMap[utils.AlertCRDName]
//		if ok && isEnabledAlert {
//			log.Errorf("Custom Resource Definition '%s' already exists...", utils.AlertCRDName)
//		} else if !ok && isEnabledAlert {
//			// create CRD
//			crds = append(crds, utils.AlertCRDName)
//			newCrds = append(newCrds, utils.AlertCRDName)
//		} else {
//			// check whether the CRD can be deleted
//			err := isDeleteCrd(utils.AlertCRDName, namespace)
//			if err != nil {
//				log.Warn(err)
//			} else {
//				// delete CRD
//				deleteCrd(utils.AlertCRDName, namespace)
//			}
//			// remove it from crds, so that the CRD controller won't run
//			crds = utils.RemoveFromStringSlice(crds, utils.AlertCRDName)
//		}
//	}
//
...
//		_, ok := crdMap[utils.BlackDuckCRDName]
//		if ok && isEnabledBlackDuck {
//			log.Errorf("Custom Resource Definition '%s' already exists...", utils.BlackDuckCRDName)
//		} else if !ok && isEnabledBlackDuck {
//			// create CRD
//			crds = append(crds, utils.BlackDuckCRDName)
//			newCrds = append(newCrds, utils.BlackDuckCRDName)
//		} else {
//			// check whether the CRD can be deleted
//			err := isDeleteCrd(utils.BlackDuckCRDName, namespace)
//			if err != nil {
//				log.Warn(err)
//			} else {
//				// delete CRD
//				deleteCrd(utils.BlackDuckCRDName, namespace)
//			}
//			// remove it from crds, so that the CRD controller won't run
//			crds = utils.RemoveFromStringSlice(crds, utils.BlackDuckCRDName)
//		}
//	}
//
...
//		_, ok := crdMap[utils.OpsSightCRDName]
//		if ok && isEnabledOpsSight {
//			log.Errorf("Custom Resource Definition '%s' already exists...", utils.OpsSightCRDName)
//		} else if !ok && isEnabledOpsSight {
//			// create CRD
//			crds = append(crds, utils.OpsSightCRDName)
//			newCrds = append(newCrds, utils.OpsSightCRDName)
//		} else {
//			// check whether the CRD can be deleted
//			err := isDeleteCrd(utils.OpsSightCRDName, namespace)
//			if err != nil {
//				log.Warn(err)
//			} else {
//				// delete CRD
//				deleteCrd(utils.OpsSightCRDName, namespace)
//			}
//			// remove it from crds, so that the CRD controller won't run
//			crds = utils.RemoveFromStringSlice(crds, utils.OpsSightCRDName)
//		}
//	}
//
...
func updateAlert(alt *synopsysv1.Alert, flagset *pflag.FlagSet) (*synopsysv1.Alert, error) {
	updateAlertCobraHelper.SetCRSpec(alt.Spec)
	alertInterface, err := updateAlertCobraHelper.GenerateCRSpecFromFlags(flagset)
	if err != nil {
		return nil, err
	}
	newSpec := alertInterface.(synopsysv1.AlertSpec)
	newSpec.Environs = utils.MergeEnvSlices(newSpec.Environs, alt.Spec.Environs)
	alt.Spec = newSpec
	return alt, nil
}
...
var updateAlertCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		mockMode := cmd.Flags().Lookup("mock").Changed
		alertName, alertNamespace, _, err := getInstanceInfo(false, utils.AlertCRDName, utils.AlertName, namespace, args[0])
		if err != nil {
			return err
		}
		currAlert, err := utils.GetAlert(restClient, alertNamespace, alertName, metav1.GetOptions{})
		if err != nil {
		...
		_, err = utils.CheckAndUpdateNamespace(kubeClient, utils.AlertName, alertNamespace, alertName, newAlert.Spec.Version, false)
		if err != nil {
			return err
		}
		// Update the Alert
		_, err = utils.UpdateAlert(restClient, newAlert)
		if err != nil {
...
func updateBlackDuck(bd *synopsysv1.Blackduck, flagset *pflag.FlagSet) (*synopsysv1.Blackduck, error) {
	...
	newSpec := blackDuckInterface.(synopsysv1.BlackduckSpec)
	// merge environs
	newSpec.Environs = utils.MergeEnvSlices(newSpec.Environs, bd.Spec.Environs)
	bd.Spec = newSpec
	return bd, nil
}
...
var updateBlackDuckCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		mockMode := cmd.Flags().Lookup("mock").Changed
		blackDuckName, blackDuckNamespace, _, err := getInstanceInfo(false, utils.BlackDuckCRDName, utils.BlackDuckName, namespace, args[0])
		if err != nil {
			return err
		}
		currBlackDuck, err := utils.GetBlackduck(restClient, blackDuckNamespace, blackDuckName, metav1.GetOptions{})
		if err != nil {
		...
		// update the namespace label if the version of the app got changed
		_, err = utils.CheckAndUpdateNamespace(kubeClient, utils.BlackDuckName, blackDuckNamespace, blackDuckName, currBlackDuck.Spec.Version, false)
		if err != nil {
			return err
		}
		// Update Black Duck
		_, err = utils.UpdateBlackduck(restClient, currBlackDuck)
		if err != nil {
...
var updateBlackDuckRootKeyCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		_, _, crdScope, err := getInstanceInfo(false, utils.BlackDuckCRDName, utils.BlackDuckName, namespace, args[0])
		if err != nil {
			return err
		}

		operatorNamespace, err := utils.GetOperatorNamespaceByCRDScope(kubeClient, utils.BlackDuckCRDName, v1beta1.ResourceScope(crdScope), namespace)
		if err != nil {
		...
		blackducks, err := utils.ListBlackduck(restClient, operatorNamespace, metav1.ListOptions{})
		if err != nil {
			return fmt.Errorf("unable to list Black Duck instances in namespace '%s' due to %+v", operatorNamespace, err)
		}

		secret, err := utils.GetSecret(kubeClient, operatorNamespace, "blackduck-secret")
		if err != nil {
		...
		for _, blackduck := range blackducks.Items {
			...
			uploadCachePod, err := utils.FilterPodByNamePrefixInNamespace(kubeClient, blackDuckNamespace, util.GetResourceName(blackDuckName, utils.BlackDuckName, "uploadcache"))
			if err != nil {
				return fmt.Errorf("unable to filter the upload cache pod in namespace '%s' due to %+v", blackDuckNamespace, err)
			}

			// Create the exec into Kubernetes pod request
			req := utils.CreateExecContainerRequest(kubeClient, uploadCachePod, "/bin/sh")
		...
		_, err = utils.UpdateSecret(kubeClient, operatorNamespace, secret)
		if err != nil {
...
func updateBlackDuckAddEnviron(bd *synopsysv1.Blackduck, environ string) (*synopsysv1.Blackduck, error) {
	bd.Spec.Environs = utils.MergeEnvSlices(strings.Split(environ, ","), bd.Spec.Environs)
	return bd, nil
}
...
var updateBlackDuckAddEnvironCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		mockMode := cmd.Flags().Lookup("mock").Changed
		blackDuckName, blackDuckNamespace, _, err := getInstanceInfo(false, util.BlackDuckCRDName, utils.BlackDuckName, namespace, args[0])
		if err != nil {
			return err
		}
		currBlackDuck, err := utils.GetBlackduck(restClient, blackDuckNamespace, blackDuckName, metav1.GetOptions{})
		...
		_, err = utils.UpdateBlackduck(restClient, newBlackDuck)
		if err != nil {
...
func updateBlackDuckSetImageRegistry(bd *synopsysv1.Blackduck, imageRegistry string) (*synopsysv1.Blackduck, error) {
...
//func updateOpsSight(ops *synopsysv1.OpsSight, flagset *pflag.FlagSet) (*synopsysv1.OpsSight, error) {
...
//		return nil, err
//	}
//	newSpec := opsSightInterface.(synopsysv1.OpsSightSpec)
//	ops.Spec = newSpec
//	return ops, nil
//}
...
//	RunE: func(cmd *cobra.Command, args []string) error {
//		mockMode := cmd.Flags().Lookup("mock").Changed
//		opsSightName, opsSightNamespace, _, err := getInstanceInfo(false, util.OpsSightCRDName, utils.OpsSightName, namespace, args[0])
//		if err != nil {
//			return err
//		}
//		currOpsSight, err := utils.GetOpsSight(opsSightClient, opsSightNamespace, opsSightName)
//		if err != nil {
...
func updatePolaris(polaris polarisMultiCR, flagset *pflag.FlagSet) (*polarisMultiCR, error) {

	updateBlackDuckCobraHelper.SetCRSpec(polarisMultiCRSpec{
		authSpec:      &polaris.auth.Spec,
		polarisDBSpec: &polaris.polarisDB.Spec,
		polarisSpec:   &polaris.polaris.Spec,
	})
	...
	newSpec := spec.(polarisMultiCRSpec)
	polaris.polaris.Spec = *newSpec.polarisSpec
	polaris.polarisDB.Spec = *newSpec.polarisDBSpec
	polaris.auth.Spec = *newSpec.authSpec
	return &polaris, nil
}
...
var updatePolarisCmd = &cobra.Command{
	Use:           "polaris NAME",
	Example:       "synopsyctl update polaris <name>",
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		mockMode := cmd.Flags().Lookup("mock").Changed
		polarisName, polarisNamespace, _, err := getInstanceInfo(false, utils.PolarisCRDName, utils.PolarisName, namespace, args[0])
		if err != nil {
			return err
		}

		currMultiCrPolaris := &polarisMultiCR{}

		currMultiCrPolaris.polaris, err = utils.GetPolaris(restClient, polarisNamespace, polarisName, metav1.GetOptions{})
		if err != nil {
			return fmt.Errorf("error getting Polaris '%s' in namespace '%s' due to %+v", polarisName, polarisNamespace, err)
		}

		currMultiCrPolaris.polarisDB, err = utils.GetPolarisDB(restClient, polarisNamespace, polarisName, metav1.GetOptions{})
		if err != nil {
			return fmt.Errorf("error getting PolarisDB '%s' in namespace '%s' due to %+v", polarisName, polarisNamespace, err)
		}

		currMultiCrPolaris.auth, err = utils.GetAuthServer(restClient, polarisNamespace, polarisName, metav1.GetOptions{})
		if err != nil {
			return fmt.Errorf("error getting Authserver '%s' in namespace '%s' due to %+v", polarisName, polarisNamespace, err)
		}

		currMultiCrPolaris, err = updatePolaris(*currMultiCrPolaris, cmd.Flags())
		if err != nil {
			return err
		}
		...
		// If mock mode, return and don't create resources
		if mockMode {
			log.Debugf("generating updates to the CRD for Polaris '%s' in namespace '%s'...", polarisName, polarisNamespace)
			//return PrintResource(*currBlackDuck, mockFormat, false)
		}

		log.Infof("updating Polaris '%s' in namespace '%s'...", polarisName, polarisNamespace)
		if _, err := utils.UpdatePolaris(restClient, currMultiCrPolaris.polaris); err != nil {
			return err
		}

		if _, err := utils.UpdatePolarisDB(restClient, currMultiCrPolaris.polarisDB); err != nil {
			return err
		}

		if _, err := utils.UpdateAuthServer(restClient, currMultiCrPolaris.auth); err != nil {
			return err
		}

		log.Infof("successfully submitted updates to Polaris '%s' in namespace '%s'", polarisName, polarisNamespace)
		return nil
	},
}
...
func init() {
	...
	updateBlackDuckCobraHelper = NewBlackduckCRSpecBuilderFromCobraFlags()
	//updateOpsSightCobraHelper = opssight.NewCRSpecBuilderFromCobraFlags()
	updateAlertCobraHelper = NewAlertCRSpecBuilderFromCobraFlags()
	updatePolarisCobraHelper = NewPolarisCRSpecBuilderFromCobraFlags()

...
func getEnabledCrds() ([]string, error) {
	crds := []string{}
	if isEnabledPolaris {
		crds = append(crds, utils.PolarisCRDName)
		crds = append(crds, utils.PolarisDBCRDName)
		crds = append(crds, utils.AuthServerCRDName)
	}
	if isEnabledAlert {
...
func deployCrds(namespace string, isClusterScoped bool, crdConfigs []*apiextensions.CustomResourceDefinition) error {
	...
	for _, crdConfig := range crdConfigs {
		...
		if _, err := apiExtensionClient.ApiextensionsV1beta1().CustomResourceDefinitions().Create(crdConfig); err != nil {
			return err
		}
	}
...
func getCrdConfigs(namespace string, isClusterScoped bool, crds []string) ([]*apiextensions.CustomResourceDefinition, error) {
	crdConfigs := []*apiextensions.CustomResourceDefinition{}

	for _, crd := range crds {
		var crdConfig *apiextensions.CustomResourceDefinition
		var crdScope apiextensions.ResourceScope
		if isClusterScoped {
			crdScope = apiextensions.ClusterScoped
		} else {
			crdScope = apiextensions.NamespaceScoped
		}
...
func checkUpdateCustomResource(crdType string, namespace string, scope apiextensions.ResourceScope, isClusterScoped bool) (bool, error) {
	crd, err := apiExtensionClient.ApiextensionsV1beta1().CustomResourceDefinitions().Get(crdType, v1.GetOptions{})
	if err != nil {
...
func getSpecToDeploySOperator(crds []string) (*soperator.SpecConfig, error) {
	...
	cert, key, err := utils.GeneratePemSelfSignedCertificateAndKey(pkix.Name{
	...
	soperatorSpec := soperator.NewSOperator(operatorNamespace, synopsysOperatorImage, exposeUI, soperator.KubernetesClusterType,
...
var deployCmd = &cobra.Command{
	...
	Args: func(cmd *cobra.Command, args []string) error {
		...
		isValid := utils.IsExposeServiceValid(exposeUI)
		if !isValid {
			cmd.Help()
			return fmt.Errorf("expose ui must be '%s', '%s', '%s' or '%s'", utils.NODEPORT, utils.LOADBALANCER, utils.OPENSHIFT, utils.NONE)
		}

		isValid = utils.IsExposeServiceValid(exposeMetrics)
		if !isValid {
			cmd.Help()
			return fmt.Errorf("expose metrics must be '%s', '%s', '%s' or '%s'", utils.NODEPORT, utils.LOADBALANCER, utils.OPENSHIFT, utils.NONE)
		}
		return nil
	},
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		//if err := utils.WaitForCRD(utils.SizeCRDName, time.Second, time.Minute*3, apiExtensionClient); err != nil {
		//	return err
		//}
...
func addOperatorDeployFlags(cmd *cobra.Command) {
	...
	cmd.Flags().BoolVarP(&isEnabledPolaris, "enable-polaris", "p", isEnabledPolaris, "Enable/Disable Polaris Custom Resource Definition (CRD) in your cluster")

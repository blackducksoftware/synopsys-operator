...
func (i *InitDatabaseUpdater) Run(ch <-chan struct{}) {
	...
	lw := &cache.ListWatch{
		ListFunc: func(options metav1.ListOptions) (runtime.Object, error) {
			return i.HubClient.SynopsysV1().Blackducks(i.Config.Namespace).List(options)
		},
		WatchFunc: func(options metav1.ListOptions) (watch.Interface, error) {
			return i.HubClient.SynopsysV1().Blackducks(i.Config.Namespace).Watch(options)
		},
	}
...
func (i *InitDatabaseUpdater) deleteChannel(name string) {
	if stopCh, ok := i.Hubs[name]; ok {
		...
		log.Errorf("unable to stop Blackduck %s: not found", name)
	}
}
...
// startInitDatabaseUpdater will check every 3 minutes for Blackduck postgres restart, if so, then initialize the DB
	...
	go func() {
		...
		for {
			...
			case <-time.After(time.Duration(i.Config.PostgresRestartInMins) * time.Minute):
				...
				db, err := database.NewDatabase(hostName, "bds_hub", "postgres", postgresPassword, "postgres")

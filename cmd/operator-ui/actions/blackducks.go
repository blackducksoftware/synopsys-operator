...
func NewBlackduckResource(protoformDeployer *protoform.Deployer) (*BlackducksResource, error) {
	...
	return &BlackducksResource{protoformDeployer: protoformDeployer, blackDuckClient: blackDuckClient}, nil
}
...
func (v BlackducksResource) List(c buffalo.Context) error {
	SetVersion(c)
	blackducks, err := util.ListBlackDucks(v.blackDuckClient, v.protoformDeployer.Config.Namespace)
	if err != nil {
...
func (v BlackducksResource) New(c buffalo.Context) error {
	...
	if v.protoformDeployer.Config.IsClusterScoped {
		blackduck.Spec.Namespace = ""
	} else {
		blackduck.Spec.Namespace = v.protoformDeployer.Config.Namespace
	}
...
func (v BlackducksResource) common(c buffalo.Context, bd *blackduckapi.Blackduck, isEdit bool) error {
	...
	if bd.View.StorageClasses == nil {
		...
		storageClasses, err := util.ListStorageClasses(v.protoformDeployer.KubeClient)
		if err != nil {
	...
	blackducks, _ := util.ListBlackDucks(v.blackDuckClient, v.protoformDeployer.Config.Namespace)
	// Clone Black Ducks
	...
	if bd.View.SupportedVersions == nil {
		bd.View.SupportedVersions = apps.NewApp(&protoform.Deployer{}).Blackduck().Versions()
...
func (v BlackducksResource) Create(c buffalo.Context) error {
	...
	operatorNamespace, err := util.GetOperatorNamespaceByCRDScope(v.protoformDeployer.KubeClient, util.BlackDuckCRDName, scope, blackduck.Spec.Namespace)
	if err != nil {
		return v.redirect(c, blackduck, err)
	}

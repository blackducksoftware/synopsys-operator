...
type MetaReconcilerInterface interface {
	...
	GetInstructionManual(interface{}) (*RuntimeObjectDependencyYaml, error)
}
...
func MetaReconcile(owner metav1.Object, mri MetaReconcilerInterface) (ctrl.Result, error) {
	...
	mapOfuniqueIDToDesiredRuntimeObject, err := mri.GetRuntimeObjects(owner)
	if err != nil {
		return ctrl.Result{}, err
	}

	// get the instruction manual
	instructionManual, err := mri.GetInstructionManual(owner)
	if err != nil {
		return ctrl.Result{}, err
	}

	// make a copy of the cr
	copyOfOwner := owner.(runtime.Object).DeepCopyObject()
	metaOfCopyOfOwner := copyOfOwner.(metav1.Object)

	// hand-off to the scheduler
	err = scheduleResources(givenClient, metaOfCopyOfOwner, mapOfuniqueIDToDesiredRuntimeObject, instructionManual)
	if err != nil {
		return ctrl.Result{}, err
	}
	return ctrl.Result{}, nil
...
func CheckForReadiness(myClient client.Client, desiredRuntimeObject runtime.Object) error {
	...
	case *corev1.Pod:
		livePod := &corev1.Pod{}
		return wait.PollImmediate(time.Second*10, time.Minute*10, func() (done bool, err error) {
			_ = myClient.Get(context.TODO(), key, livePod)
			fmt.Println(livePod.Name)
			if isPodReady(livePod) == nil {
				return true, nil
			}
			return false, nil
		})
	...
	case *corev1.Service:
		...
		return wait.PollImmediate(time.Second*10, time.Minute*10, func() (done bool, err error) {
			...
			fmt.Println(liveService.Name)
			if isServiceReady(liveService) == nil {
				return true, nil
			}
			return false, nil
		})
	...
	case *corev1.ReplicationController:
		...
		return wait.PollImmediate(time.Second*10, time.Minute*10, func() (done bool, err error) {
			...
			fmt.Println(liveReplicationController.Name)
			if isReplicationControllerReady(liveReplicationController) == nil {
				return true, nil
			}
			return false, nil
		})
	...
	case *appsv1.Deployment:
		...
		return wait.PollImmediate(time.Second*10, time.Minute*10, func() (done bool, err error) {
			...
			fmt.Println(liveDeployment.Name)
			if isDeploymentReady(liveDeployment) == nil {
				return true, nil
			}
			return false, nil
		})
	...
	case *batchv1.Job:
		...
		return wait.PollImmediate(time.Second*10, time.Minute*10, func() (done bool, err error) {
			...
			fmt.Println(liveJob.Name)
			if isJobCompleted(liveJob) == nil {
				return true, nil
			}
			return false, nil
		})
	...
	case *appsv1.StatefulSet:
		...
		return wait.PollImmediate(time.Second*10, time.Minute*10, func() (done bool, err error) {
			...
			fmt.Println(liveStatefulSet.Name)
			if isStatefulSetReady(liveStatefulSet) == nil {
				return true, nil
			}
			return false, nil
		})

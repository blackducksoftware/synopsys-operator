...
func getInstanceInfo(mock bool, crdName string, appName string, namespace string, name string) (string, string, apiextensions.ResourceScope, error) {
	...
	if !mock {
		crd, err := apiExtensionClient.ApiextensionsV1beta1().CustomResourceDefinitions().Get(crdName, metav1.GetOptions{})
		if err != nil {
			return "", "", "", fmt.Errorf("unable to get Custom Resource Definition '%s' in your cluster due to %+v", crdName, err)
		}
		crdScope = apiextensions.ResourceScope(crd.Spec.Scope)
	}
	...
	if crdScope == apiextensions.ClusterScoped {
		if len(namespace) == 0 {
			...
			if len(appName) > 0 {
				ns, err := kubeClient.CoreV1().Namespaces().List(metav1.ListOptions{LabelSelector: fmt.Sprintf("synopsys.com/%s.%s", appName, name)})
				if err != nil {

...
var createPolarisReportingCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		out, err := util.RunHelm3("install", kubeConfigPath, "", namespace, polarisReportingName, polarisReportingChartRepository, helmValuesMap)
		if err != nil {
			return fmt.Errorf("failed to create Polaris-Reporting resources: %+v; %+v", out, err)
		}
...
var createPolarisReportingNativeCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		out, err := util.RunHelm3("template", kubeConfigPath, "", namespace, polarisReportingName, polarisReportingChartRepository, helmValuesMap)
		if err != nil {

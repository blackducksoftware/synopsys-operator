...
func TestAddSpecFlags(t *testing.T) {
	...
	expectedCmd.Flags().StringVar(&ctl.ExposeService, "expose-service", ctl.ExposeService, "Type of Service to Expose")
	...
	var tests = []struct {
		...
		{flagName: "expose-service",
			...
			changedCtl: &Ctl{Spec: &alertapi.AlertSpec{},
				...
				CfsslImageVersion:    "",
				StandAlone:           true,
				ExposeService:        "changedExposeService",
				Port:                 0,
				EncryptionPassword:   "",
				EncryptionGlobalSalt: "",
				Environs:             []string{},
			...
			changedSpec: &alertapi.AlertSpec{ExposeService: "changedExposeService"},
		},
		// case
		{flagName: "port",
			initialCtl: NewAlertCtl(),

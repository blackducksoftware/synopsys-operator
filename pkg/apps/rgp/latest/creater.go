...
import (
	...
	containers "github.com/blackducksoftware/synopsys-operator/pkg/apps/rgp/latest/containers"
...
type Creater struct {
	...
	commonConfig *crdupdater.CommonConfig
	dryRun       bool
	isPatched    bool
}
...
func (c *Creater) Update(spec *v1.RgpSpec) error {
	...
	updater := crdupdater.NewUpdater(c.dryRun, c.isPatched)
	// add, patch or remove the replication controllers
	deploy, err := crdupdater.NewDeployment(c.commonConfig, gr.GetDeployments())
	if err != nil {
		return errors.Annotatef(err, "unable to create the deployment object:")
	}
	updater.AddUpdater(deploy)

	svc, err := crdupdater.NewService(c.commonConfig, gr.GetServices())
	if err != nil {

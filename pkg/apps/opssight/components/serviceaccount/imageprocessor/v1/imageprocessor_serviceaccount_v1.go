...
func init() {
	store.Register(types.OpsSightImageProcessorServiceAccountV1, NewOpsSightServiceAccount)
}
...
func (o *OpsSightServiceAccount) GetServiceAccount() (*components.ServiceAccount, error) {
	if !o.opsSight.Spec.Perceiver.EnableImagePerceiver || (o.opsSight.Spec.Perceiver.EnablePodPerceiver && o.opsSight.Spec.Perceiver.EnableImagePerceiver) {
		return nil, nil
	}

	name := o.opsSight.Spec.Perceiver.ServiceAccount
	...
	serviceAccount.AddLabels(map[string]string{"component": name, "app": "opssight", "name": o.opsSight.Name})

...
func (hc *Creater) init(deployer *horizon.Deployer, bdspec *v1.BlackduckSpec, hubContainerFlavor *containers.ContainerFlavor) error {
	...
	deployer.AddComponent(horizonapi.ServiceAccountComponent, util.CreateServiceAccount(bdspec.Namespace, bdspec.Namespace))

	// Create a cluster role binding and associated it to a service account
	deployer.AddComponent(horizonapi.ClusterRoleBindingComponent, util.CreateClusterRoleBinding(bdspec.Namespace, bdspec.Namespace, bdspec.Namespace, "", "ClusterRole", "synopsys-operator-admin"))
	...
	if bdspec.PersistentStorage {
		for _, claim := range bdspec.PVC {
			...
			deployer.AddComponent(horizonapi.PersistentVolumeClaimComponent, pvc)

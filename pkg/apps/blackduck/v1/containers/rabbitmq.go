...
func (c *Creater) GetRabbitmqDeployment(imageName string) (*components.ReplicationController, error) {
	...
	return util.CreateReplicationControllerFromContainer(
		&horizonapi.ReplicationControllerConfig{Namespace: c.hubSpec.Namespace, Name: util.GetResourceName(c.name, "rabbitmq", c.config.IsClusterScoped), Replicas: util.IntToInt32(1)},
		&util.PodConfig{
...
func (c *Creater) getRabbitmqVolumes() []*components.Volume {
	...
	if c.hubSpec.PersistentStorage {
		rabbitmqDataEmptyDir, _ = util.CreatePersistentVolumeClaimVolume("dir-rabbitmq-data", util.GetResourceName(c.name, "blackduck-rabbitmq", c.config.IsClusterScoped))
	} else {
...
func (c *Creater) GetRabbitmqService() *components.Service {
	return util.CreateService(util.GetResourceName(c.name, "rabbitmq", c.config.IsClusterScoped), c.GetVersionLabel("rabbitmq"), c.hubSpec.Namespace, rabbitmqPort, rabbitmqPort, horizonapi.ServiceTypeServiceIP, c.GetVersionLabel("rabbitmq"))
}

...
func checkPVCs(defaultPVCs []*horizoncomponents.PersistentVolumeClaim, expectedPVCs []blackduckapi.PVC, observedPVCs []*horizoncomponents.PersistentVolumeClaim) error {
	...
	for expectedPVCName, expectedPVC := range expectedPVCMap {
		observedPVCObj := observedPVCMap[expectedPVCName].GetObj()
		defaultPVCObj := defaultPVCMap[expectedPVCName].GetObj()
		if len(expectedPVC.Size) > 0 {
			if expectedPVC.Size != observedPVCObj.Storage {
				return fmt.Errorf("invalid set storage size - expected %+v, got %+v", expectedPVC.Size, observedPVCObj.Storage)
			}
		} else {
			if observedPVCObj.Storage != defaultPVCObj.Storage {
				return fmt.Errorf("invalid default storage size - expected %+v, got %+v", expectedPVC.Size, observedPVCObj.Storage)
			}
		}
		if len(expectedPVC.StorageClass) > 0 {
			if expectedPVC.StorageClass != *observedPVCObj.StorageClass {
				return fmt.Errorf("invalid storageClass - expected %+v, got %+v", expectedPVC.StorageClass, *observedPVCObj.StorageClass)
			}
		}
	}

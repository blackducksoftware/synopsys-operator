...
func (c *Creater) GetWebappLogstashDeployment(webappImageName string, logstashImageName string) (*components.ReplicationController, error) {
	...
	return util.CreateReplicationControllerFromContainer(&horizonapi.ReplicationControllerConfig{Namespace: c.hubSpec.Namespace, Name: "webapp-logstash", Replicas: util.IntToInt32(1)},
		"", []*util.Container{webappContainerConfig, logstashContainerConfig}, c.getWebappLogtashVolumes(),
		initContainers, []horizonapi.PodAffinityConfig{}, c.GetVersionLabel("webapp-logstash"), c.GetLabel("webapp-logstash"), c.hubSpec.RegistryConfiguration.PullSecrets)
}

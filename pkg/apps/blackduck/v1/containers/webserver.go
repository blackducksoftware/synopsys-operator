...
func (c *Creater) GetWebServerNodePortService() *components.Service {
	return util.CreateService(util.GetResourceName(c.name, "webserver-exposed", c.config.IsClusterScoped), c.GetLabel("webserver"), c.hubSpec.Namespace, int32(443), webserverPort, horizonapi.ServiceTypeNodePort, c.GetLabel("webserver-np"))
}
...
func (c *Creater) GetWebServerLoadBalancerService() *components.Service {
	return util.CreateService(util.GetResourceName(c.name, "webserver-exposed", c.config.IsClusterScoped), c.GetLabel("webserver"), c.hubSpec.Namespace, int32(443), webserverPort, horizonapi.ServiceTypeLoadBalancer, c.GetLabel("webserver-lb"))
}

...
func (c *Creater) GetSolrDeployment(imageName string) (*components.ReplicationController, error) {
	...
	return util.CreateReplicationControllerFromContainer(
		&horizonapi.ReplicationControllerConfig{Namespace: c.hubSpec.Namespace, Name: util.GetResourceName(c.name, "solr", c.config.IsClusterScoped), Replicas: util.IntToInt32(1)},
		&util.PodConfig{
...
func (c *Creater) getSolrVolumes() []*components.Volume {
	...
	if c.hubSpec.PersistentStorage {
		solrVolume, _ = util.CreatePersistentVolumeClaimVolume("dir-solr", util.GetResourceName(c.name, "blackduck-solr", c.config.IsClusterScoped))
	} else {
...
func (c *Creater) GetSolrService() *components.Service {
	return util.CreateService(util.GetResourceName(c.name, "solr", c.config.IsClusterScoped), c.GetLabel("solr"), c.hubSpec.Namespace, solrPort, solrPort, horizonapi.ServiceTypeServiceIP, c.GetVersionLabel("solr"))
}

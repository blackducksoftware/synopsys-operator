...
func (c *Creater) GetSecrets(cert string, key string, sealKey []byte) []*components.Secret {
	...
	uploadCacheSecret := components.NewSecret(horizonapi.SecretConfig{Namespace: c.hubSpec.Namespace, Name: util.GetResourceName(c.name, "upload-cache", c.config.IsClusterScoped), Type: horizonapi.SecretTypeOpaque})

...
func (c *Creater) GetSecrets(cert string, key string, sealKey []byte) []*components.Secret {
	...
	certificateSecret := components.NewSecret(horizonapi.SecretConfig{Namespace: c.hubSpec.Namespace, Name: util.GetResourceName(c.name, "webserver-certificate", c.isClusterScope), Type: horizonapi.SecretTypeOpaque})
	...
	if len(c.hubSpec.ProxyCertificate) > 0 {
		...
		if err != nil {
			...
			proxyCertificateSecret := components.NewSecret(horizonapi.SecretConfig{Namespace: c.hubSpec.Namespace, Name: util.GetResourceName(c.name, "proxy-certificate", c.isClusterScope), Type: horizonapi.SecretTypeOpaque})
	...
	if len(c.hubSpec.AuthCustomCA) > 0 {
		...
		if err != nil {
			...
			authCustomCASecret := components.NewSecret(horizonapi.SecretConfig{Namespace: c.hubSpec.Namespace, Name: util.GetResourceName(c.name, "auth-custom-ca", c.isClusterScope), Type: horizonapi.SecretTypeOpaque})
	...
	uploadCacheSecret := components.NewSecret(horizonapi.SecretConfig{Namespace: c.hubSpec.Namespace, Name: util.GetResourceName(c.name, "upload-cache", c.isClusterScope), Type: horizonapi.SecretTypeOpaque})

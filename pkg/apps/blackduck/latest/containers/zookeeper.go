...
func (c *Creater) GetZookeeperDeployment(imageName string) (*components.ReplicationController, error) {
	...
	return util.CreateReplicationControllerFromContainer(&horizonapi.ReplicationControllerConfig{Namespace: c.hubSpec.Namespace, Name: "zookeeper", Replicas: util.IntToInt32(1)}, "",
		[]*util.Container{zookeeperContainerConfig}, c.getZookeeperVolumes(), initContainers, []horizonapi.PodAffinityConfig{}, c.GetVersionLabel("zookeeper"), c.GetLabel("zookeeper"), c.hubSpec.RegistryConfiguration.PullSecrets)
}

...
func (p *SpecConfig) PerceptorMetricsDeployment() (*components.Deployment, error) {
	...
	deployment := components.NewDeployment(horizonapi.DeploymentConfig{
		Replicas:  &replicas,
		Name:      utils.GetResourceName(p.opssight.Name, util.OpsSightName, "prometheus"),
...
func (p *SpecConfig) perceptorMetricsVolumes() ([]*components.Volume, error) {
	...
	vols = append(vols, components.NewConfigMapVolume(horizonapi.ConfigMapOrSecretVolumeConfig{
		...
		MapOrSecretName: utils.GetResourceName(p.opssight.Name, util.OpsSightName, "prometheus"),
...
func (p *SpecConfig) PerceptorMetricsNodePortService() (*components.Service, error) {
	service := components.NewService(horizonapi.ServiceConfig{
		Name:      utils.GetResourceName(p.opssight.Name, util.OpsSightName, "prometheus-exposed"),
		Namespace: p.opssight.Spec.Namespace,
...
func (p *SpecConfig) PerceptorMetricsConfigMap() (*components.ConfigMap, error) {
	...
	targets := []string{
		fmt.Sprintf("%s:%d", utils.GetResourceName(p.opssight.Name, util.OpsSightName, p.opssight.Spec.Perceptor.Name), p.opssight.Spec.Perceptor.Port),
		fmt.Sprintf("%s:%d", utils.GetResourceName(p.opssight.Name, util.OpsSightName, p.opssight.Spec.ScannerPod.Scanner.Name), p.opssight.Spec.ScannerPod.Scanner.Port),
		fmt.Sprintf("%s:%d", utils.GetResourceName(p.opssight.Name, util.OpsSightName, p.opssight.Spec.ScannerPod.ImageFacade.Name), p.opssight.Spec.ScannerPod.ImageFacade.Port),
	}
	if p.opssight.Spec.Perceiver.EnableImagePerceiver {
		targets = append(targets, fmt.Sprintf("%s:%d", utils.GetResourceName(p.opssight.Name, util.OpsSightName, p.opssight.Spec.Perceiver.ImagePerceiver.Name), p.opssight.Spec.Perceiver.Port))
	}
	if p.opssight.Spec.Perceiver.EnablePodPerceiver {
		targets = append(targets, fmt.Sprintf("%s:%d", utils.GetResourceName(p.opssight.Name, util.OpsSightName, p.opssight.Spec.Perceiver.PodPerceiver.Name), p.opssight.Spec.Perceiver.Port))
	}
	if p.opssight.Spec.EnableSkyfire {
		targets = append(targets, fmt.Sprintf("%s:%d", utils.GetResourceName(p.opssight.Name, util.OpsSightName, p.opssight.Spec.Skyfire.Name), p.opssight.Spec.Skyfire.PrometheusPort))
	}
...
func (p *SpecConfig) GetPrometheusOpenShiftRoute() *corev1.Route {
	...
	if strings.ToUpper(p.opssight.Spec.Perceptor.Expose) == util.OPENSHIFT {
		return &corev1.Route{
			...
			Namespace:          namespace,
			Kind:               "Service",
			ServiceName:        utils.GetResourceName(p.opssight.Name, util.OpsSightName, p.opssight.Spec.Prometheus.Name),

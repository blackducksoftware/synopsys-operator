...
func validateConfigMaps(t *testing.T, configMaps []*components.ConfigMap, opssightSpec *opssightapi.OpsSightSpec) {
	...
	for _, cm := range configMaps {
		...
		if !strings.EqualFold(cm.ConfigMap.Name, actualConfigMap.name) {
			t.Errorf("config map name is not equal. Expected: %s, Actual: %s", actualConfigMap.name, cm.ConfigMap.Data)
		}

		if !strings.EqualFold(cm.GetName(), prometheus) {
			if _, ok := cm.ConfigMap.Data[actualConfigMap.fileName]; !ok {
				t.Errorf("config map file name is not equal. Expected: %s, Actual: %s", actualConfigMap.fileName, cm.ConfigMap.Data[actualConfigMap.fileName])
			}
		}
	}
}
